...
const rowOpenAIAccountPool = document.getElementById('row-openai-account-pool');
const selectOpenAIAccountSource = document.getElementById('select-openai-account-source');
const inputOpenAIAccountPoolImport = document.getElementById('input-openai-account-pool-import');
const btnOpenAIAccountPoolRefresh = document.getElementById('btn-openai-account-pool-refresh');
const btnOpenAIAccountPoolClearUsed = document.getElementById('btn-openai-account-pool-clear-used');
const btnOpenAIAccountPoolDeleteAll = document.getElementById('btn-openai-account-pool-delete-all');
const btnOpenAIAccountPoolImport = document.getElementById('btn-openai-account-pool-import');
const openAIAccountPoolSummary = document.getElementById('openai-account-pool-summary');
const openAIAccountPoolList = document.getElementById('openai-account-pool-list');
const btnOpenAIAccountPoolMarkUnused = document.getElementById('btn-openai-account-pool-mark-unused');
const btnOpenAIAccountPoolEnable = document.getElementById('btn-openai-account-pool-enable');
const btnOpenAIAccountPoolDisable = document.getElementById('btn-openai-account-pool-disable');
...
let openAIAccountPoolEntriesState = [];
...
function collectSettingsPayload() {
  ...
  return {
    ...
    openaiAccountSource: String(
      (typeof selectOpenAIAccountSource !== 'undefined' && selectOpenAIAccountSource
        ? selectOpenAIAccountSource.value
        : undefined)
      ?? latestState?.openaiAccountSource
      ?? ''
    ).trim().toLowerCase() === 'imported-pool' ? 'imported-pool' : '',
    // Sidepanel state is deliberately password-free. Do not run it through the
    // credential-bearing normalizer here, or every safe account row is dropped
    // before the next SAVE_SETTING can merge its password server-side.
    openaiAccountPoolEntries: typeof (typeof window !== 'undefined' ? window : globalThis)
      ?.OpenAIAccountPoolUtils?.normalizeOpenAIAccountListEntries === 'function'
      ? (typeof window !== 'undefined' ? window : globalThis).OpenAIAccountPoolUtils.normalizeOpenAIAccountListEntries(
        typeof openAIAccountPoolEntriesState !== 'undefined' ? openAIAccountPoolEntriesState : []
      )
      : [],
...
function applySettingsState(state) {
  ...
  const accountPoolUtils = typeof window !== 'undefined' ? window : globalThis;
  const restoredOpenAIAccountPoolEntries = typeof accountPoolUtils?.OpenAIAccountPoolUtils?.normalizeOpenAIAccountListEntries === 'function'
    ? accountPoolUtils.OpenAIAccountPoolUtils.normalizeOpenAIAccountListEntries(state?.openaiAccountPoolEntries)
    : typeof accountPoolUtils?.OpenAIAccountPoolUtils?.normalizeOpenAIAccounts === 'function'
      ? accountPoolUtils.OpenAIAccountPoolUtils.normalizeOpenAIAccounts(state?.openaiAccountPoolEntries)
    : [];
  if (typeof openAIAccountPoolEntriesState !== 'undefined') {
    openAIAccountPoolEntriesState = restoredOpenAIAccountPoolEntries;
  }
  if (typeof selectOpenAIAccountSource !== 'undefined' && selectOpenAIAccountSource) {
    selectOpenAIAccountSource.value = state?.openaiAccountSource === 'imported-pool' ? 'imported-pool' : '';
  }
  if (typeof openAIAccountPoolManager !== 'undefined') {
    openAIAccountPoolManager?.render(restoredOpenAIAccountPoolEntries);
  }
...
function updatePanelModeUI() {
  ...
  if (typeof rowOpenAIAccountPool !== 'undefined' && rowOpenAIAccountPool) {
    ...
    if (
      !eligible
      && typeof selectOpenAIAccountSource !== 'undefined'
      && selectOpenAIAccountSource?.value
    ) {
      selectOpenAIAccountSource.value = '';
    }
...
const openAIAccountPoolManager = window.SidepanelOpenAIAccountPoolManager?.createOpenAIAccountPoolManager({
  dom: {
    row: rowOpenAIAccountPool,
    source: selectOpenAIAccountSource,
    importInput: inputOpenAIAccountPoolImport,
    refreshButton: btnOpenAIAccountPoolRefresh,
    clearButton: btnOpenAIAccountPoolClearUsed,
    deleteAllButton: btnOpenAIAccountPoolDeleteAll,
    importButton: btnOpenAIAccountPoolImport,
    summary: openAIAccountPoolSummary,
    list: openAIAccountPoolList,
    markUnusedButton: btnOpenAIAccountPoolMarkUnused,
    enableButton: btnOpenAIAccountPoolEnable,
    disableButton: btnOpenAIAccountPoolDisable,
  },
  state: {
    getEntries: () => openAIAccountPoolEntriesState,
    setEntries: (entries) => { openAIAccountPoolEntriesState = OpenAIAccountPoolUtils.normalizeOpenAIAccountListEntries(entries); },
  },
  ...
  actions: {
    ...
    importEntries: async (text) => {
      ...
      openAIAccountPoolEntriesState = OpenAIAccountPoolUtils.normalizeOpenAIAccountListEntries(result.accounts);
      markSettingsDirty(true);
      await saveSettings({ silent: true });
      return { accounts: openAIAccountPoolEntriesState, feedback: result.feedback };
    },
  },
});
openAIAccountPoolManager?.bindEvents();
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  switch (message.type) {
    ...
    case 'DATA_UPDATED': {
      ...
      if (message.payload.openaiAccountPoolEntries !== undefined || message.payload.openaiAccountSource !== undefined) {
        if (message.payload.openaiAccountPoolEntries !== undefined) {
          openAIAccountPoolEntriesState = typeof OpenAIAccountPoolUtils?.normalizeOpenAIAccountListEntries === 'function'
            ? OpenAIAccountPoolUtils.normalizeOpenAIAccountListEntries(message.payload.openaiAccountPoolEntries)
            : [];
        }
        if (message.payload.openaiAccountSource !== undefined && selectOpenAIAccountSource) {
          selectOpenAIAccountSource.value = message.payload.openaiAccountSource === 'imported-pool' ? 'imported-pool' : '';
        }
        openAIAccountPoolManager?.render(openAIAccountPoolEntriesState);
      }
