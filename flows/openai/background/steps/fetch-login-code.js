(function attachBackgroundStep8(root, factory) {
  ...
  function createStep8Executor(deps = {}) {
    ...
    async function submitImportedAccountOtpCode(state, visibleStep) {
      ...
      if (typeof submitVerificationCode !== 'function') {
        throw new Error(`步骤 ${visibleStep}：OTP 验证码提交能力未接入。`);
      }
      await addLog(`步骤 ${visibleStep}：检测到导入账号的 OTP Secret，正在生成并提交 6 位验证码。`, 'info');
      const code = await accountPoolUtils.generateTotpCode(otpSecret);
      const submitResult = await submitVerificationCode(8, code, { completionStep: visibleStep });
      if (submitResult?.invalidCode) {
        throw new Error(`步骤 ${visibleStep}：OTP 验证码被页面拒绝，请确认本机时间与 OTP Secret。`);
      }
      await setState({ loginVerificationRequestedAt: null, step8VerificationTargetEmail: '' });
      if (typeof completeNodeFromBackground === 'function') {
        await completeNodeFromBackground(state?.nodeId || 'fetch-login-code', {
    ...
    async function runStep8Attempt(state, runtime = {}) {
      ...
      if (await submitImportedAccountOtpCode(state, visibleStep)) {
        return { lastResendAt: Number(state?.loginVerificationRequestedAt) || 0 };
      }
