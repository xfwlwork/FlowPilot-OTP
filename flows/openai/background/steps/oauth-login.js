(function attachBackgroundStep7(root, factory) {
  ...
  function createStep7Executor(deps = {}) {
    ...
    function redactStep7ErrorMessage(error, secret = '') {
      const message = String(getErrorMessage(error) || '未知错误');
    ...
    function buildStep7CompletionPayload(result = {}, currentState = {}, currentIdentifierType = '', currentPhoneNumber = '') {
      ...
      const importedAccountId = String(currentState?.currentOpenAIAccountId || '').trim();
      if (
        String(currentState?.openaiAccountSource || '').trim().toLowerCase() === 'imported-pool'
        && importedAccountId
      ) {
        // The OTP secret remains in the credential-bearing pool. Step 8 only
        // receives this runtime account ID and resolves the secret locally.
        payload.currentOpenAIAccountId = importedAccountId;
      }
    ...
    async function executeStep7(state) {
      ...
      if (
        usesImportedAccountPool
        && !resolveForcedStep7IdentifierType(initialState)
        && !isStep7BoundEmailReloginContext(initialState)
      ) {
        if (!selectedImportedAccountId) {
          ...
          initialState = {
            ...initialState,
            currentOpenAIAccountId: selectedImportedAccountId,
          };
          await setState?.({ currentOpenAIAccountId: selectedImportedAccountId });
        }
        if (typeof resolveImportedAccountCredentials !== 'function') {
          throw new Error('OpenAI 账号池当前不可用，无法解析登录账号。');
        }
        const importedCredentials = await resolveImportedAccountCredentials(initialState);
        const importedEmail = String(importedCredentials?.email || '').trim();
        ...
        initialState = {
          ...
          accountIdentifier: importedEmail,
          email: importedEmail,
      ...
      const forceEmailLoginForThisRun = shouldForceStep7EmailLogin(initialState);
      ...
      while (attempt < STEP6_MAX_ATTEMPTS) {
        ...
        try {
          const rawCurrentState = {
            ...(attempt === 1 ? initialState : await getState()),
            // getState may not contain runtime-only imported credentials on retries.
            ...(usesImportedAccountPool
              && !resolveForcedStep7IdentifierType(initialState)
              && !isStep7BoundEmailReloginContext(initialState) ? {
              currentOpenAIAccountId: initialState.currentOpenAIAccountId,
              accountIdentifierType: initialState.accountIdentifierType,
              accountIdentifier: initialState.accountIdentifier,
          ...
          await addLog(`第 ${attempt} 次尝试失败，原因：${redactStep7ErrorMessage(err, initialState.password)}；准备重试...`, 'warn', {
            step: completionStep,
            stepKey: 'oauth-login',
          });
        }
      ...
      throw new Error(`步骤 ${completionStep}：判断失败后已重试 ${STEP6_MAX_ATTEMPTS - 1} 次，仍未成功。最后原因：${redactStep7ErrorMessage(lastError, initialState.password)}`);
    }
