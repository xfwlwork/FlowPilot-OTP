(function attachMultiPageSettingsSchema(root, factory) {
  ...
  function createSettingsSchema(deps = {}) {
    ...
    function normalizeSettingsState(input = {}, options = {}) {
      ...
      const normalized = {
        ...
        openaiAccountSource: String(
          input?.openaiAccountSource
          ?? nested?.openaiAccountSource
          ?? defaults.openaiAccountSource
        ).trim().toLowerCase() === 'imported-pool' ? 'imported-pool' : '',
        openaiAccountPoolEntries: Array.isArray(
          input?.openaiAccountPoolEntries
          ?? nested?.openaiAccountPoolEntries
          ?? defaults.openaiAccountPoolEntries
        )
          ? (input?.openaiAccountPoolEntries
            ?? nested?.openaiAccountPoolEntries
            ?? defaults.openaiAccountPoolEntries).map((entry) => ({
            id: String(entry?.id || ''),
            email: String(entry?.email || '').trim().toLowerCase(),
            password: String(entry?.password || ''),
            otpSecret: String(entry?.otpSecret || '').trim(),
    ...
    function buildSettingsView(settingsState = {}, baseInput = {}) {
      ...
      next.openaiAccountPoolEntries = cloneValue(normalizedState.openaiAccountPoolEntries || []);
