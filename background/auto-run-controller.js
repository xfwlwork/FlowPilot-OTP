      ...
      for (let targetRun = resumeCurrentRun; targetRun <= totalRuns; targetRun += 1) {
        ...
        while (attemptRun <= maxAttemptsForRound) {
          ...
          try {
            ...
            if (isFullWorkflowDone(completedState) && typeof markCurrentRegistrationAccountUsed === 'function') {
              await markCurrentRegistrationAccountUsed(completedState, {
                logPrefix: '自动运行：完整流程成功后',
                level: 'ok',
              });
            }
