(function attachBackgroundMessageRouter(root, factory) {
  ...
  function createMessageRouter(deps = {}) {
    const {
      ...
      projectPublicState = (value) => value,
    ...
    async function handleMessage(rawMessage, sender) {
      ...
      switch (message.type) {
        ...
        case 'SAVE_SETTING': {
          ...
          const payloadForPersistence = { ...rawPayload };
          if (Object.prototype.hasOwnProperty.call(rawPayload, 'openaiAccountPoolEntries')
            && typeof mergeOpenAIAccountPoolEntries === 'function') {
            payloadForPersistence.openaiAccountPoolEntries = mergeOpenAIAccountPoolEntries(
              currentState?.openaiAccountPoolEntries,
              rawPayload.openaiAccountPoolEntries
            );
          }
          const updates = buildPersistentSettingsPayload(payloadForPersistence);
          ...
          if (Object.keys(stateUpdates).length > 0 && typeof broadcastDataUpdate === 'function') {
            broadcastDataUpdate(projectPublicState(stateUpdates));
          }
          if (modeChanged) {
        ...
        case 'IMPORT_OPENAI_ACCOUNT_POOL': {
          if (typeof importOpenAIAccountPoolEntries !== 'function') {
            throw new Error('OpenAI 账号池导入能力未接入。');
          }
          const result = await importOpenAIAccountPoolEntries(message.payload?.text || '');
          return { ok: true, ...result };
        }
