...
test('settings schema preserves imported OpenAI account OTP secrets for the runtime login flow', () => {
  const { settingsSchema } = loadApis();
  const schema = settingsSchema.createSettingsSchema();
  const normalized = schema.normalizeSettingsState({
  ...
  assert.equal(normalized.openaiAccountPoolEntries[0].otpSecret, 'JBSWY3DPEHPK3PXP');
});
