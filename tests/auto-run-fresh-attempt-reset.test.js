...
let currentState = {
  ...
  currentOpenAIAccountId: 'imported-account-1',
...
async function resetState() {
  ...
  currentState = {
    ...
    openaiAccountPoolEntries: (prev.openaiAccountPoolEntries || []).map((entry) => ({ ...entry })),
...
async function runAutoSequenceFromStep() {
  ...
  if (runCalls === 2) {
    if (state.openaiAccountPoolEntries?.[0]?.email !== 'imported@example.com') {
      throw new Error('fresh round must preserve imported OpenAI account pool');
    }
    if (state.currentOpenAIAccountId !== '') {
      throw new Error('fresh round must clear selected imported OpenAI account');
    }
    if (state.email !== null) {
