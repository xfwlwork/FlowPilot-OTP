...
test('step 8 resolves the selected imported account OTP secret and submits its current code', async () => {
  const calls = { otpStates: [], generatedSecrets: [], submissions: [], completions: [] };
  globalScope.MultiPageOpenAIAccountPoolUtils = {
    generateTotpCode: async (secret) => {
      calls.generatedSecrets.push(secret);
      return '654321';
    },
  };
  const executor = api.createStep8Executor({
    ...
    ensureStep8VerificationPageReady: async () => ({ state: 'verification_page' }),
    getImportedAccountOtpSecret: (state) => {
      calls.otpStates.push(state);
      return state.currentOpenAIAccountId === 'otp-account' ? 'JBSWY3DPEHPK3PXP' : '';
    },
    getOAuthFlowStepTimeoutMs: async (timeout) => timeout,
    getTabId: async () => 1,
    submitVerificationCode: async (step, code, options) => {
      calls.submissions.push({ step, code, options });
      return {};
    },
    completeNodeFromBackground: async (_nodeId, payload) => calls.completions.push(payload),
    setState: async () => {},
    throwIfStopped: () => {},
  });
  ...
  assert.equal(calls.otpStates[0].currentOpenAIAccountId, 'otp-account');
  assert.deepEqual(calls.generatedSecrets, ['JBSWY3DPEHPK3PXP']);
  assert.deepEqual(calls.submissions, [{ step: 8, code: '654321', options: { completionStep: 8 } }]);
