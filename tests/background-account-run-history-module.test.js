...
test('account run history never persists imported-pool passwords, while regular records remain compatible', () => {
  const source = fs.readFileSync('background/account-run-history.js', 'utf8');
  ...
  const imported = helpers.buildAccountRunHistoryRecord({
    email: 'imported@example.com', password: 'must-not-persist', openaiAccountSource: 'imported-pool',
  }, 'failed', '登录失败');
  assert.equal(imported.password, '');
  const normalizedImported = helpers.normalizeAccountRunHistoryRecord({
    email: 'imported@example.com', password: 'must-not-persist', openaiAccountSource: 'imported-pool',
    finalStatus: 'failed', failureDetail: '登录失败',
  });
  assert.equal(normalizedImported.password, '');

  const regular = helpers.buildAccountRunHistoryRecord({ email: 'regular@example.com', password: 'compatible-password' }, 'success');
  assert.equal(regular.password, 'compatible-password');
});
