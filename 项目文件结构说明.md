...
- `background/sub2api-api.js`：SUB2API 管理接口直连模块，负责后台登录 SUB2API、按 `openai / grok` 平台读取并校验分组、为侧栏连接测试返回服务端真实分组、按配置选择代理、生成 OpenAI OAuth 地址、交换 localhost callback 授权码并创建账号；ChatGPT Session 与 Agent Identity 共用 Codex 凭据预检、导入 payload、结果计数和敏感错误脱敏边界，主链路不依赖 SUB2API 后台页面 DOM 自动化。
...
- `tests/sidepanel-sub2api-connection-test.test.js`：测试 OpenAI / Grok 共用的 SUB2API 连接测试入口、消息接线以及连接成功后的分组同步渲染。
